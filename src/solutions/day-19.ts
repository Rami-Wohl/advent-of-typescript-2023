...
type BuildTuple<
	L extends number,
	T extends Stuff[number],
	R extends Stuff[number][] = []
> = R extends { length: L } ? R : BuildTuple<L, T, [...R, T]>;

export type Rebuild<
	A extends number[],
	R extends unknown[] = [],
	T extends Stuff[number] = "🛹"
> = A extends [infer N extends number, ...infer L extends number[]]
	? T extends "🛹"
		? Rebuild<L, [...R, ...BuildTuple<N, T>], "🚲">
		: T extends "🚲"
		? Rebuild<L, [...R, ...BuildTuple<N, T>], "🛴">
		: T extends "🛴"
		? Rebuild<L, [...R, ...BuildTuple<N, T>], "🏄">
